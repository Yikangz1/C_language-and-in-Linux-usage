...
#include <string.h>
  ...
  #define qqq2_address 0x7fff0cfa4030
...
printf("the size of bs: %d\n",bs);
 ...
 printf("the size of i1: %d\n",i1);
  ...
  printf("bool2:%d\n",bool2);
  //********************************************************************************************************//
  //********************************pointer & pointer's pointer(begin)  **********************************//
  //******************************************************************************************************//
  int a2=0x1F;
  printf("a2: %p\n",a2);// print varibal a2

  int *ppp = &a2;
  printf("a2: %p\n",*ppp); //print varibal a2
  printf("a2 address(ppp): %p\n",ppp);//print varibal a2's address or variable ppp


  int *qqq = &ppp;
  printf("ppp : %p\n",*qqq);  //print ppp, which was stored in address qqq
  printf("ppp address(qqq): %p\n",qqq); // print ppp's address or variable qqq  
  // note; you can not use qqq to index a[2], as qqq's type is int* instead of int**. by defining int** to store
  // varibale ppp's adress, you can use int**(pointer's pointer) to index a2. PLS SEE the next two line codes:
  int **qqq2 =&ppp;// once you defined variable qqq2, which tyoe is "int**", you can use "**qqq" in the next line.
  printf("a2:(pointer's pointer method):%p\n",**qqq2);
  printf("a2's address:(pointer's pointer method):%p\n",*qqq2);
//  printf("a2:(pointer's pointer method):%p\n",**qqq);  //note: you can not use "**qqq", because variable qqq type is "int*" instead of "int**"

  //printf("a2:(pointer's pointer method):%p\n",qqq2_address);
//  printf("a2:(pointer's pointer method):%p\n",*(volatile uint64_t *)(qqq2_address));
 
  //************ print a2 value, three methods (begin)*****************//
  printf("a2(direct method): %p\n",a2);// print varibal a2
  printf("a2(pointer method): %p\n",*ppp);// print varibal a2
  printf("a2(pointer's pointer method): %p\n",**qqq2);// print varibal a2
  //printf("a2(pointer's pointer method): %p\n",**qqq);// note: this manner is wrong, as qqq type is (int*),can not use (**) 


  //********************************************************************************************************//
  //********************************pointer & pointer's pointer(end)  **********************************//
  //******************************************************************************************************//
